...
void ctrlGetInput(void)
{
    ...
    GINPUT input = {};
#ifndef EDUKE32
    static unsigned int lastInputTicks = 0;
    const int hiTicksPerSec = 1000;
    unsigned int currentHiTicks = timerGetTicks();
    unsigned int elapsedInputTicks = currentHiTicks - lastInputTicks;
    if (elapsedInputTicks > kTicsPerSec) elapsedInputTicks = kTicsPerSec;

    lastInputTicks = currentHiTicks;

    #define scaleAdjustmentToInterval(x) ((x) * kTicsPerSec * elapsedInputTicks / hiTicksPerSec)
#else
    ...
    if (turnLeft)
#ifndef EDUKE32
        input.q16turn = fix16_ssub(input.q16turn, fix16_from_int(scaleAdjustmentToInterval(ClipHigh(12 * turnHeldTime, gTurnSpeed)>>2)));
#else
        input.q16turn = fix16_ssub(input.q16turn, fix16_from_float(scaleAdjustmentToInterval(ClipHigh(12 * turnHeldTime, gTurnSpeed)>>2)));
#endif
    if (turnRight)
#ifndef EDUKE32
        input.q16turn = fix16_sadd(input.q16turn, fix16_from_int(scaleAdjustmentToInterval(ClipHigh(12 * turnHeldTime, gTurnSpeed)>>2)));
#else
